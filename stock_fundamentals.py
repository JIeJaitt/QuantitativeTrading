"""
股票/ETF基本面数据获取工具
使用akshare获取指定股票或ETF的30日基本面数据，并输出为Markdown格式
...
def get_etf_history(stock_code: str, days: int = 30) -> pd.DataFrame:
    ...
    Returns:
        历史行情DataFrame
    """
    # 方法1: 尝试使用 stock_zh_a_hist (也支持 ETF)
    try:
        end_date = datetime.now().strftime("%Y%m%d")
        ...
        print(f"  stock_zh_a_hist 获取失败: {e}")

    # 方法2: 尝试使用 fund_etf_hist_em
    try:
        end_date = datetime.now().strftime("%Y%m%d")
        start_date = (datetime.now() - timedelta(days=days + 15)).strftime("%Y%m%d")
        ...
        print(f"  fund_etf_hist_em 获取失败: {e}")

    # 方法3: 尝试使用 fund_etf_hist_sina
    try:
        # 深圳 ETF 前缀为 sz，上海为 sh
        prefix = "sz" if stock_code.startswith(("15", "16")) else "sh"
        df = ak.fund_etf_hist_sina(symbol=f"{prefix}{stock_code}")
        if not df.empty:
            # 统一列名
            df = df.rename(
                columns={
        ...
        print(f"  fund_etf_hist_sina 获取失败: {e}")

    print("获取ETF历史行情数据失败: 所有方法均失败")
    return pd.DataFrame()
...
def get_etf_realtime_quote(stock_code: str) -> dict:
    ...
    try:
        ...
        print(f"  fund_etf_spot_em 获取失败: {e}")
...
def calculate_technical_indicators(df: pd.DataFrame) -> dict:
    ...
    # 量比 = 当日成交量 / 过去5日平均成交量
    if len(volume) >= 5:
        avg_volume_5 = volume.iloc[-6:-1].mean()  # 过去5日平均
...
| 成交量 | {realtime.get('成交量', 'N/A')} |
| 成交额 | {realtime.get('成交额', 'N/A')} |
| 换手率 | {realtime.get('换手率', 'N/A')}% |
| 市盈率(动态) | {realtime.get('市盈率-动态', 'N/A')} |
...
"""
    ...
    if not history_df.empty:
        for _, row in history_df.iterrows():
            ...
            turnover_str = turnover if turnover != "N/A" else "N/A"
